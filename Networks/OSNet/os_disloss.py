...
class decoder(nn.Module):
    def __init__(self):
        ...
        self.ghost = ghost(384,384)
        # self.rcab = RCAB(384)
        # self.ean = ean_module(384)
        # self.fam = FAM(384)
        ...
        self.proj = Projector(384)
        self.loss = MomentDistanceLoss()
    ...
    def forward(self, x): 
        ...
        proj = self.proj(identy)
        disloss = self.loss(proj)
...
class MomentDistanceLoss(nn.Module):
    def __init__(self):
        super(MomentDistanceLoss, self).__init__()
    ...
    def forward(self, x):
        ...
        target_mean = torch.randn(N, device=x.device) 
        target_var = torch.abs(torch.randn(N, device=x.device)) 
        
        mean_distance = F.mse_loss(mean, target_mean)
        
        var_distance = F.mse_loss(var, target_var)
        
        loss = (mean_distance + var_distance) / 2
