...
class OSNet(BaseNet):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.experts2 = nn.Sequential(
                nn.Conv2d(192, 512, kernel_size=1, padding=0),
                nn.ReLU(),
            )
        self.experts3 = nn.Sequential(
                nn.Conv2d(256, 512, kernel_size=1, padding=0),
                nn.ReLU(),
            )
        self.experts4 = nn.Sequential(
                nn.Conv2d(256, 512, kernel_size=1, padding=0),
                nn.ReLU(),
            )
    ...
    def forward(self, x):
        ...
        experts.append(x)
        x = self.maxpool(x)
        x2 = self.conv2(x)      #torch.Size([2, 256, 32, 32])
        experts.append(x2)
        x3 = self.conv3(x2)    #torch.Size([2, 384, 16, 16])
        experts.append(x3)
        x4 = self.conv4(x3)     #torch.Size([2, 512, 16, 16])
        experts.append(x4)
        x5 = self.conv5(x4)      #torch.Size([2, 512, 16, 16])
        experts.append(x5)
        ...
        for i in range(len(experts1)):
            ...
            if i==1:
                ...
                experts1[i] = output
            if i==2:
                output = self.experts3(experts1[i])
                experts1[i] = output
            if i==3:
                output = self.experts4(experts1[i])
                experts1[i] = output
        ...
        # experts.append(combined_output)
